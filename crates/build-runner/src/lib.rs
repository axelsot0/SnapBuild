...
mod tests {
    ...
    #[test]
    fn runs_successful_command_portably() {
        let cwd = std::env::temp_dir();
        let result = run_command(&cwd, "rustc", &["--version"]).unwrap();
    ...
    #[test]
    fn captures_failed_command_portably() {
        let cwd = std::env::temp_dir();
        let result = run_command(&cwd, "rustc", &["--definitely-not-a-real-flag"]).unwrap();
        assert!(!result.success);
